...
int main() {
    ...
    // Wait a moment for the terminal to open before sending the command
    std::this_thread::sleep_for(std::chrono::seconds(1));

    // Step 4: SEND COMMAND - Execute the coordinator command
    send_command_to_tmux("COORDINATOR", "./coordinator -p 9090");

    // Wait a moment to ensure the coordinator starts properly
    std::this_thread::sleep_for(std::chrono::seconds(1));
    ...
    // Wait 2 seconds before sending additional commands to the CLIENT session
